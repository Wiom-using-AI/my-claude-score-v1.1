...
if (flags.help) {
  console.log(`
  my-claude-score v1.1 — Your Claude Code Score
  ...
  Scores:
    Claude Score (0-100)   Combined headline score
  ...
  Speed Labels:
    ...
    Progressing (25-49) Building skills gradually
    Warming Up (0-24)   Early in the journey
...
const combinedScore = calculateCombinedScore(
  scoring.totalScore,
  speedResult ? speedResult.score : null
);
...
if (flags.json) {
  const output = {
    ...
    speed: {
      ...
      calendarDays: speedResult.calendarDays,
      activeDays: speedResult.activeDays,
      timeMode: speedResult.timeMode,
      expected: speedResult.expected,
      firstSessionDate: speedResult.firstSessionDate,
    },
    summary: `My Claude Code Score: ${combinedScore} (Proficiency ${scoring.totalScore}, Speed ${speedResult.score !== null ? speedResult.score : 'N/A'})`,
  };
  console.log(JSON.stringify(output, null, 2));
} else if (flags.badge) {
  console.log(renderBadge(scoring, tier, archetype, speedResult, combinedScore));
} else {
  console.log(render(scoring, tier, archetype, paths, flags.verbose, speedResult, combinedScore));
}
