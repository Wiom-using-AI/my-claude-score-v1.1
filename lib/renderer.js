...
const SIGNAL_PRIORITY = {
  'Project-level CLAUDE.md':    { impact: 1, minTier: 'Apprentice' },
  'Rules quality':              { impact: 1, minTier: 'Apprentice' },
  'Security rules file':        { impact: 1, minTier: 'Apprentice' },
  'Active plans':               { impact: 1, minTier: 'Apprentice' },
  'Plugins enabled':            { impact: 1, minTier: 'Apprentice' },
  'MCP servers configured':     { impact: 1, minTier: 'Apprentice' },
  'No hardcoded secrets':       { impact: 1, minTier: 'Apprentice' },
  'Claude co-authored commits': { impact: 1, minTier: 'Apprentice' },

  'Custom slash commands':      { impact: 2, minTier: 'Craftsperson' },
  'Settings customization':     { impact: 2, minTier: 'Craftsperson' },
  'Usage activity':             { impact: 2, minTier: 'Craftsperson' },
  'Commit message quality':     { impact: 2, minTier: 'Craftsperson' },
  'Hooks configured':           { impact: 2, minTier: 'Craftsperson' },
  'Agent depth':                { impact: 2, minTier: 'Craftsperson' },
  'Todo usage':                 { impact: 2, minTier: 'Craftsperson' },
  'Strategic plugin bonus':     { impact: 2, minTier: 'Craftsperson' },
  'Skills available':           { impact: 2, minTier: 'Craftsperson' },
  'Centralized credential references': { impact: 2, minTier: 'Craftsperson' },
  'Security-adjacent plugins':  { impact: 2, minTier: 'Craftsperson' },
  'Custom agents':              { impact: 2, minTier: 'Craftsperson' },
  'Team/swarm usage':           { impact: 2, minTier: 'Craftsperson' },
  'Agent memory populated':     { impact: 2, minTier: 'Craftsperson' },

  'Keybindings customized':     { impact: 3, minTier: 'Artisan' },
  'Launch configs':             { impact: 3, minTier: 'Artisan' },
  'Semantic commit prefixes':   { impact: 3, minTier: 'Artisan' },
  'Model diversity':            { impact: 3, minTier: 'Artisan' },
  'Plugin blocklist curation':  { impact: 3, minTier: 'Artisan' },
  'Plugin blocklist with reasons': { impact: 3, minTier: 'Artisan' },
  'Team complexity':            { impact: 3, minTier: 'Artisan' },
  'Workflow explorer':          { impact: 3, minTier: 'Artisan' },

  'Custom permissions':         { impact: 3, minTier: 'Virtuoso' },
  'Security awareness':         { impact: 3, minTier: 'Virtuoso' },
};

const TIER_RANK = {
  'Apprentice': 0, 'Craftsperson': 1, 'Artisan': 2, 'Virtuoso': 3, 'Sage': 4,
};
 ...
 * v12: adds speed score display in header.
 ...
 * @returns {string}
 */
function render(scoring, tier, archetype, paths, verbose = false, speedResult = null) {
  const lines = [];
  ...
  // ── Speed score (v12) ──────────────────────────────────────────
  if (speedResult) {
    const speedSection = renderSpeedSection(speedResult, scoring.totalScore);
    for (const sl of speedSection) lines.push(sl);
    lines.push(emptyLine());
  }
...
function getSpeedInterpretation(proficiency, speedResult) {
  ...
  if (proficiency >= 65 && speed < 50) {
    return '\u2139\uFE0F  High mastery, steady pace \u2014 consistent long-term learner';
  }
  // Low proficiency + high speed → fast starter, still ramping up
  if (proficiency < 35 && speed >= 65) {
...
// ── Speed section renderer (v12) ────────────────────────────────────
function renderSpeedSection(speedResult, proficiencyScore) {
  const lines = [];

  if (speedResult.score === null) {
    // Too early or no data
    lines.push(line(`  ${fg.gray('\u23F3  Learning Speed: Measuring... (need more usage data)')}`));
    return lines;
  }

  const labelColor = speedLabelColor(speedResult.label);
  const speedBar = progressBar(speedResult.score, 100, 16);

  // Line 1: Speed score with label
  const speedStr = `  ${speedResult.emoji}  ${labelColor(style.bold(speedResult.label))}  ${fg.gray('\u2502')}  ${fg.brightWhite(style.bold(String(speedResult.score)))}${fg.gray('/100')}  ${speedBar}`;
  lines.push(line(speedStr));

  // Line 2: Context — how many days, expected vs actual
  const dayLabel = 'active days';
  const contextParts = [];
  contextParts.push(`${speedResult.effectiveDays} ${dayLabel}`);
  if (speedResult.expected !== null) {
    contextParts.push(`expected: ${speedResult.expected}`);
    contextParts.push(`actual: ${proficiencyScore}`);
  }
  lines.push(line(`  ${fg.gray('  ' + contextParts.join('  \u2502  '))}`));

  // Line 3 (optional): Contextual interpretation for dual-score dissonance
...
function collectStats(scoring, paths, speedResult) {
  ...
  // Speed time mode (v12)
  if (speedResult && speedResult.effectiveDays) {
    const mode = speedResult.timeMode === 'active' ? 'active' : 'cal';
    parts.push(`Time: ${speedResult.effectiveDays}d (${mode})`);
  }
